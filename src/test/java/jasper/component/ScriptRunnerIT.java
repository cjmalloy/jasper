...
print(yaml.dump({
  'ref': [output],
	...
	@Test
	void testShellYaml() throws UntrustedScriptException {
		// language=Bash
		var upperCaseScript = """
ref=$(cat)
title=$(echo "$ref" | yq eval '.title' -)
comment=$(echo "$ref" | yq eval '.comment' -)
url=$(echo "$ref" | yq eval '.url' -)
cat <<EOF
ref:
- url: "comment:$(uuidgen)"
  sources: ["${url}"]
  title: "Re: ${title}"
  comment: "${comment^^}"  # Convert to uppercase
  tags:
  - public
  - +needle
EOF
