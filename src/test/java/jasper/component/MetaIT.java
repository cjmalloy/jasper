...
public class MetaIT {
	...
	void testCreateMetadata() {
		...
		assertThat(ref.getMetadata().getPluginCounts()).isEmpty();
	...
	void testCreateMetadataResponse() {
		...
		assertThat(parent.get().getMetadata().getPluginCounts()).isNullOrEmpty();
	...
	void testCreateMetadataPluginResponse() {
		...
		assertThat(parent.get().getMetadata().getInternalResponses()).containsExactly(URL+2);
		assertThat(parent.get().getMetadata().getPluginCounts().get("plugin/comment")).isEqualTo(1);
	}
