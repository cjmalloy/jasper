...
public class TokenProviderImpl extends AbstractJwtTokenProvider {
	...
	public TokenProviderImpl(Props props, UserRepository userRepository, SecurityMetersService securityMetersService) {
		...
		for (var c : props.getSecurity().clientList()) {
			...
			keys.put(client, key);
			jwtParser.put(client, Jwts.parser().verifyWith(key).build());
		}
	...
	public String createToken(Authentication authentication, boolean rememberMe) {
		...
		return Jwts
			.builder()
			.claim(props.getSecurity().getClient("").getUsernameClaim(), authentication.getName())
			.claim("aud", props.getSecurity().getClient("").getAuthentication().getJwt().getClientId())
	...
	public Authentication getAuthentication(String token, String origin) {
		var claims = jwtParser.get(origin).parseSignedClaims(token).getPayload();
