...
public class ScrapeService {
	...
	public void scrape(String url) throws URISyntaxException {
		webScraper.fetch(url);
	}

	@Timed(value = "jasper.service", extraTags = {"service", "fetch"}, histogram = true)
	public byte[] fetch(String url) throws URISyntaxException {
		if (webScraper.exists(url)) {
			return webScraper.fetch(url).getData();
		}
		// Only require role for new scrapes
		if (!auth.hasRole(USER)) throw new AccessDeniedException("Requires USER role to scrape.");
		return webScraper.fetch(url).getData();
	}
