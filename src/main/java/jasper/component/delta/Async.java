...
public class Async {
	...
	public void handleRefUpdate(Message<RefDto> message) {
		...
		try {
			...
			tags.forEach((tag, v) -> {
				logger.trace("{} Checking for Async Tag ({} {}): {} Tags: {} Whitelisted: {}", origin, tag, configs.root().script(tag, origin) ? "☑️" : "🚩️", ud.getUrl(), ud.getTags(), configs.root().getScriptSelectors());
